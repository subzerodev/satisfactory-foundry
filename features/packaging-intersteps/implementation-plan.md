...
- [ ] Write failing tests for all 12 bundled reversible pairs. Pin package ID,
...
- [ ] Add optional `interstep` to `StageLink`, but do not expose it through
...
- [ ] Run `npm test -- --run src/data/packaging.test.ts` and `npm run check`.
...
- [ ] First write failing tests for Water at `10,600/min`: 177 package, 89
  unpackage, 10,600 packaged, 10,600 empty returns, 2,660 MW, 9 Mk6 lanes each.
- [ ] Add Nitrogen supply/demand tests proving material values remain in gas
...
- [ ] Write failing v8 tests before production edits: current save/load,
...
- [ ] Freeze v7 types. Add `PlanLinkV8`/`PlanFileV8`; all current APIs and store
  writer constructors emit v8. Old builds must reject v8 rather than erase
  interstep intent.
- [ ] Implement closed-world raw-intent validators. Mode/trip/fuel discriminants,
...
- [ ] Implement v7 migration by rebuilding every accepted legacy transport arm
